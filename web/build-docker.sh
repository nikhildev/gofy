...
show_run_instructions() {
    ...
    echo "  docker run -d -p 5050:8080 --name gofy-weather ${full_image_name}"
    echo ""
    print_info "To run with custom port (e.g., 8080 or 3000):"
    ...
    echo "  docker-compose -f docker-compose.local.yml up -d  # Runs on port 5050"
    echo ""
    ...
    echo ""
    print_info "Access the weather application at:"
    echo "  - Docker Compose: http://localhost:5050"
    echo "  - Direct run: http://localhost:5050 (or your chosen port)"
    print_info "Try different cities: http://localhost:5050/?location=London"
    echo ""
